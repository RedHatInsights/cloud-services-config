...
node {
  stage ("create backup for old YAML files") {
    ...
    if (BRANCH == "prod-stable") {
      PREFIX = ""
      STAGETESTSTR = "\'stage and stable\' --release=stable"
      PRODTESTSTR = "\'prod and stable\' --release=stable"
    } else if (BRANCH == "prod-beta") {
      PREFIX = "beta/"
      STAGETESTSTR = "\'stage and beta\' --release=beta"
      PRODTESTSTR = "\'prod and beta\' --release=beta"
    } else {
